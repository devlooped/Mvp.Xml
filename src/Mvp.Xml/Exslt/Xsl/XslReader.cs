...
public class XslReader : XmlReader
{
    ...
    readonly TokenPipe pipe;
    readonly BufferWriter writer;
    readonly ScopeManager scope;
    Thread thread;

    readonly bool multiThread;
    ...
    int attOffset; // 0 - means reader is positioned on element, when reader potitionrd on the first attribute attOffset == 1
    int attCount;
    int depth;
    ...
    public override bool Read()
    {
        ...
        switch (nodeType)
        {
            case XmlNodeType.Element:
                ...
                {
                    pipe.Read(out var attType, out var attName, out var attText);
    ...
    public override void Close() => SetUndefinedState(ReadState.Closed);
    ...
    public override string GetAttribute(int i)
    {
        if (IsInsideElement())
        {
            if (0 <= i && i < attCount)
            {
                pipe.GetToken(i + 1, out _, out var attValue);
                return attValue;
            }
        }
    ...
    static readonly char[] qnameSeparator = { ':' };
    ...
    int FindAttribute(string name)
    {
        if (IsInsideElement())
        {
            ...
            {
                pipe.GetToken(i, out var attName, out _);
    ...
    public override string GetAttribute(string name, string ns)
    {
        if (IsInsideElement())
        {
            for (var i = 1; i <= attCount; i++)
            {
                pipe.GetToken(i, out var attName, out var attValue);
    ...
    public override string LookupNamespace(string prefix) => scope.LookupNamespace(prefix);
    /// <summary>See <see cref="XmlReader.Close()"/>.</summary>
    ...
    static bool RefEquals(string strA, string strB)
    {
        Debug.Assert(
    ...
    static bool RefEquals(QName qnA, QName qnB)
    {
        Debug.Assert(
    ...
    class BufferWriter : XmlWriter
    {
        readonly QNameTable qnameTable;
        readonly TokenPipe pipe;
        string firstText;
        readonly StringBuilder sbuilder;
        QName curAttribute;
        ...
        public override void WriteEndAttribute() => pipe.Write(XmlNodeType.Attribute, curAttribute, MergeText());
        public override void WriteString(string text) => AppendText(text);

        public override void WriteFullEndElement() => WriteEndElement();
        public override void WriteRaw(string data) => WriteString(data); // In XslReader output we ignore disable-output-escaping
        ...
        class QNameTable
        {
            ...
            readonly XmlNameTable nameTable;
            readonly Dictionary<string, List<QName>> qnames = new();

            public QNameTable(XmlNameTable nameTable) => this.nameTable = nameTable;

            public QName GetQName(string local, string nsUri, string prefix)
            {
                nsUri = nameTable.Add(nsUri);
                prefix = nameTable.Add(prefix);
                if (!qnames.TryGetValue(local, out var list))
                {
                    list = new List<QName>();
                ...
                var qname = new QName(nameTable.Add(local), nsUri, prefix);
                list.Add(qname);
                return qname;
            }
    ...
    class ScopeManager
    {
        ...
        static readonly string atomLang = new("lang".ToCharArray());
        static readonly string atomSpace = new("space".ToCharArray());
        readonly XmlNameTable nameTable;
        readonly string stringEmpty;
        ...
        public void PopScope()
        {
            ...
            do
            {
                ...
                var record = records[lastRecord - 1];
        ...
        void AddRecord(QName qname)
        {
            if (lastRecord == records.Length)
            {
                var temp = new QName[records.Length * 2];
        ...
        string Space2Str(XmlSpace space) => space switch
        {
        ...
        XmlSpace Str2Space(string space) => space switch
        {
    ...
    class TokenPipe
    {
        ...
        public virtual void Reset() => ReadStartPos = ReadEndPos = WritePos = 0;
        ...
        public virtual void WriteException(Exception e) => throw e;
        ...
        public virtual void Close() => Write(XmlNodeType.None, null, null);
        ...
        public virtual void GetToken(int attNum, out QName name, out string value)
        {
            ...
            XmlToken.Get(ref Buffer[ReadStartPos + attNum], out var nodeType, out name, out value);
    ...
    class TokenPipeMultiThread : TokenPipe
    {
        Exception exception;
        ...
        {
            Debug.Assert(0 <= attNum && attNum < ReadEndPos - ReadStartPos - 1);
            XmlToken.Get(ref Buffer[(ReadStartPos + attNum) & Mask], out var nodeType, out name, out value);
            Debug.Assert(nodeType == (attNum == 0 ? XmlNodeType.Element : XmlNodeType.Attribute), "We use GetToken() only to access parts of start element tag.");
        }
    }
}
