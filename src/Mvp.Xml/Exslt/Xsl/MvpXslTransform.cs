...
public class MvpXslTransform : IXmlTransform
{
    ...
    /// </summary>
    public MvpXslTransform() => CompiledTransform = new XslCompiledTransform();

    /// <summary>
    /// Initializes a new instance of the <see cref="MvpXslTransform"/> 
    /// class with the specified debug setting. 
    /// </summary>
    public MvpXslTransform(bool debug) => CompiledTransform = new XslCompiledTransform(debug);
    ...
    public void Load(IXPathNavigable stylesheet) => LoadStylesheetFromReader(stylesheet.CreateNavigator().ReadSubtree());
    ...
    public void Load(string stylesheetUri) => LoadStylesheetFromReader(XmlReader.Create(stylesheetUri));
    ...
    public void Load(XmlReader stylesheet) => LoadStylesheetFromReader(stylesheet);
    ...
    public void Load(IXPathNavigable stylesheet, XsltSettings settings, XmlResolver stylesheetResolver) => LoadStylesheetFromReader(stylesheet.CreateNavigator().ReadSubtree(), settings, stylesheetResolver);
    ...
    /// style sheets referenced in XSLT <c>import</c> and <c>include</c> elements. If this is a 
    /// null reference (Nothing in Visual Basic), external resources are not resolved.</param>
    public void Load(string stylesheetUri, XsltSettings settings, XmlResolver stylesheetResolver) => LoadStylesheetFromReader(XmlReader.Create(stylesheetUri), settings, stylesheetResolver);
    ...
    /// style sheets referenced in XSLT <c>import</c> and <c>include</c> elements. If this is a 
    /// null reference (Nothing in Visual Basic), external resources are not resolved.</param>
    public void Load(XmlReader stylesheet, XsltSettings settings, XmlResolver stylesheetResolver) => LoadStylesheetFromReader(stylesheet, settings, stylesheetResolver);
    ...
    public XmlOutput Transform(XmlInput input, XsltArgumentList arguments, XmlOutput output)
    {
        ...
        if (output.Destination is not XmlWriter xmlWriter)
        {
            closeWriter = true;
            while (true)
            {
    ...
    {
        var command = CompiledTransform.GetType().GetField(
        ...
        var executeMethod = command.GetType().GetMethod("Execute", BindingFlags.Instance | BindingFlags.Public,
            ...
            command.GetType().GetMethod("Execute", BindingFlags.Instance | BindingFlags.Public,
                null, new[] { typeof(object), typeof(XmlResolver), typeof(XsltArgumentList), typeof(XmlWriter) }, null);
    ...
    /// </summary>        
    protected void LoadStylesheetFromReader(XmlReader reader) => LoadStylesheetFromReader(reader, XsltSettings.Default, new XmlUrlResolver());
    ...
    /// <remarks>If null is passed in then a new XsltArgumentList is constructed. </remarks>
    public static XsltArgumentList AddExsltExtensionObjects(XsltArgumentList list) => AddExsltExtensionObjects(list, ExsltFunctionNamespace.All);
