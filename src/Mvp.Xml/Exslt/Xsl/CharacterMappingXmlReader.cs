...
public class CharacterMappingXmlReader : XmlWrappingReader
{
    ...
    readonly string nxsltNamespace;
    readonly string characterMapTag;
    readonly string nameTag;
    readonly string outputCharacterTag;
    readonly string characterTag;
    readonly string stringTag;
    readonly string outputTag;
    readonly string useCharacterMapsTag;
    List<string> useCharacterMaps;
    string currMapName;
    CharacterMap currMap;

    /// <summary>
    /// Creates new instance of the <see cref="CharacterMappingXmlReader"/> with given
    /// base <see cref="XmlReader"/>.
    /// </summary>        
    public CharacterMappingXmlReader(XmlReader baseReader) : base(baseReader)
    {
    ...
    public override bool Read()
    {
        ...
        if (NodeType == XmlNodeType.Element && NamespaceURI == nxsltNamespace && LocalName == characterMapTag)
        {
            ...
            var referencedMaps = base[useCharacterMapsTag];
        ...
        else if (NodeType == XmlNodeType.Element && NamespaceURI == nxsltNamespace && LocalName == outputCharacterTag)
        {
            ...
            var character = base[characterTag];
        ...
        {
            //nxslt:output
            var useMaps = base[useCharacterMapsTag];
