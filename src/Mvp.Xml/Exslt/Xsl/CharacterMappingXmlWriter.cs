...
public class CharacterMappingXmlWriter : XmlWrappingWriter
{
    ...
    readonly CharacterMappingXmlReader reader;

    /// <summary>
    /// Creates new instance of the <see cref="CharacterMappingXmlWriter"/>
    /// with given base <see cref="XmlWriter"/> and charcter mapping.
    /// </summary>        
    public CharacterMappingXmlWriter(XmlWriter baseWriter, Dictionary<char, string> mapping) : base(baseWriter)
    ...
    /// information.
    /// </remarks>
    public CharacterMappingXmlWriter(CharacterMappingXmlReader reader, XmlWriter baseWriter) : base(baseWriter)
        => this.reader = reader;
    ...
    void FlushBuffer(StringBuilder buf)
    {
        if (buf.Length <= 0)
            return;
