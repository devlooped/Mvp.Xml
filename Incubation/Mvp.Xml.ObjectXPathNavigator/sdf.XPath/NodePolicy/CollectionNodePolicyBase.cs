...
namespace Mvp.Xml.ObjectXPathNavigator.NodePolicy
{
	...
	public abstract class CollectionNodePolicyBase : GenericNodePolicy
	{
		...
		protected static Node CreateChildNode( object child, Node node )
		{
			...
			if( child != null ) 
			{
				childNode = new Node( node.Context, node.Context.GetNodePolicy( child ));
			...
			{
				childNode = new Node( node.Context, null );
				childNode.NodeType = XPathNodeType.Comment;
				childNode.Value = "Null reference";
			}
			return childNode;
		}
	}
